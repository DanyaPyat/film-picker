...
class Movie:
    ...
    YEAR_URL = f"https://api.themoviedb.org/3/discover/movie"
    def KEY_URL(query): return f"https://api.themoviedb.org/3/search/movie?query={query}&include_adult=true&language=en-EU&page=1"
    ...
    async def film_picker_name(query)->list:
        async with aiohttp.ClientSession() as session:
            async with session.get(Movie.KEY_URL(query), headers = headers) as response:
                if response.status == 200:
                    data = json.loads(await response.content.read())
                    if 'results' in data and data['results']:
                        movie = data['results'][0]
                        genre_ids = movie.get('genre_ids',[0])
                        genre_names = await Movie.genres_convert(genre_ids)
                        return [movie['title'], movie['overview'], movie['release_date'], movie['id'], genre_names]
                    else:
                        return None
...
async def main():
    ...
    movie = await Movie.film_picker_name("super")
    print(movie)
    # print(result)
    # print(film)
    # print(year_film)
